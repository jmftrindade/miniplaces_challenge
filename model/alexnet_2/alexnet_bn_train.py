...
# tf Graph input
x = tf.placeholder(tf.float32, [None, load_size, load_size, c])
print(x)
print(tf.size(x))
keep_dropout = tf.placeholder(tf.float32)
train_phase = tf.placeholder(tf.bool)
prepro_fn = preprocessing_factory.get_preprocessing('inception_v1', is_training=is_training);
x = tf.map_fn(lambda img: prepro_fn(img, fine_size, fine_size, fast_mode=False), x);
